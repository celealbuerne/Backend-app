...
export function sanitizeInput(req: Request, res: Response, next: NextFunction) {
    ...
    const sanitizedInput: Record<string, any> = {  //arma un nuevo objeto con los datos que vienen del req.body, pero ya limpiados y convertidos al tipo que queremos
...
//si ponia nan cubria q no fuera letras, pero no q no sea negativo o 0
    if (sanitizedInput.localidadID !== undefined &&(!Number.isInteger(sanitizedInput.localidadID) || sanitizedInput.localidadID <= 0)) {
...
export function validarCrearDatos(req: Request, res: Response, next: NextFunction) {
    ...
    const camposObligatorios: (keyof CreateAeropuertoDTO)[] = [
        'nombre',
        'codigo',
        'localidadID',
    ];
